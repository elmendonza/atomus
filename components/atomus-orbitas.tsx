...
// Logo do Atomus: 4 órbitas paradas e 6 bolinhas girando ao longo delas, em loop (usado na tela de carregamento e no login).
const TAM = 260;
const C = TAM / 2;
const RX = 105;
const RY = 37;
const COR = '#1C1C1E';
const CICLO_S = 60; // todos os períodos abaixo dividem 60 s, então o loop não "pula"
...
const T0 = (() => {
  try {
    ...
    return typeof s === 'number' && isFinite(s) ? s % CICLO_S : 0;
  } catch {
    return 0;
  }
})();
...
export function AtomusOrbitas({ size = TAM, traco = 4 }: { size?: number; traco?: number }) {
  const t = useSharedValue(T0);

  useEffect(() => {
    t.value = withRepeat(withTiming(T0 + CICLO_S, { duration: CICLO_S * 1000, easing: Easing.linear }), -1);
  }, [t]);

  return (
    <Svg width={size} height={size} viewBox={`0 0 ${TAM} ${TAM}`}>
      {ORBITAS.map((rot) => (
        <G key={rot} rotation={rot} originX={C} originY={C}>
          <Ellipse cx={C} cy={C} rx={RX} ry={RY} fill="none" stroke={COR} strokeWidth={traco} />
        </G>
      ))}
      {BOLINHAS.map((b, i) => (
